...
function Dish(props) {
  ...
  useEffect(() => {
    let index = userSignedin.dishes.indexOf(props.dish.id);
    document.getElementById(`btn-03-${props.dish.id}`).style.backgroundColor =
      "transparent";
    document.getElementById(`btn-01-${props.dish.id}`).style.backgroundColor =
      "transparent";
    document.getElementById(`btn-02-${props.dish.id}`).style.backgroundColor =
      "transparent";
    switch (index) {
      case 1: {
      ...
      if (!userSignedin.dishes[4]) {
        ...
        if (prev_index_same_dish !== -1) {
          if (prev_index_same_dish === data.rank) {
            ...
            // check all rank are empty if yes add boolean value false - not filled
            if (
              userSignedin.dishes[1] &&
              userSignedin.dishes[2] &&
              userSignedin.dishes[3]
            ) {
              userSignedin.dishes[4] = false;
            }
            ...
            userSignedin.dishes[data.rank] = data.id;
            props.updateDishScore(
              { id: data.id, score: (4 - prev_index_same_dish) * 10 },
